...
import com.bombardier.plugin.testingplugin.scheduling.TempTest;
import com.bombardier.plugin.testingplugin.scheduling.TestPacket;
import com.bombardier.plugin.testingplugin.scheduling.TestScheduler;
import com.bombardier.plugin.testingplugin.statistics.Result;
import com.bombardier.plugin.testingplugin.statistics.TestResultAdapter;
import com.bombardier.plugin.testingplugin.statistics.TestSuite;
...
import com.bombardier.plugin.testingplugin.utils.HistoryAndStatsUtils;
...
public class TestBuilder extends Builder {
	...
	public TestBuilder(String testConfigCmd) {
	...
	private void addTestingPublisher(AbstractBuild<?, ?> build, Set<Node> nodes) {
		TestPublisher testPublisher = new TestPublisher(nodes);
		DescribableList<Publisher, Descriptor<Publisher>> publishers = ((TestBuild) build)
			...
			for (Executor e : c.getExecutors()) {
				...
				if (e.isIdle()) {
					...
					project.getBuildersList().add(
							new DumbTestingBuilder(projectName));
	...
	/**
	 * Used to get the {@link DumbTestingBuilder} descriptor.
	 * 
	 * @since 1.0
	 */
	@Override
